...
void measuringSelectionSort() {
	...
	tt=0;
		...
		fillArrayRandom(a,t); //Rellena el array con números aleatoreos
...
void measuringInsertionSort() {
    ...
    for (int t= INITALLA; t <= MAXTALLA; t += INCRTALLA) {
	    int a[t];
	    tt = 0;
	    for (int r = 0; r< REPETICIONESL; r++){
	    	 fillArraySortedInAscendingOrder(a,t );
	    ...
	    double tPromed = (double) tt / REPETICIONESQ;

	    tt=0;
	    for (int r=0; r<REPETICIONESQ; r++){
		fillArraySortedInDescendingOrder(a,t);
		auto ti = chrono::high_resolution_clock::now();
		insertionSort(a, t);
		auto tf = chrono::high_resolution_clock::now();
		tt += chrono::duration_cast<chrono::microseconds> (tf - ti).count();
